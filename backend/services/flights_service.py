...
def _route_coordinates(raw: dict, prefix: str) -> tuple[float | None, float | None]:
    ...
    if isinstance(raw.get(f"{prefix}_lat"), bool) or isinstance(
        raw.get(f"{prefix}_lng"), bool
    ):
        return None, None
    return coordinates({
    ...
    *,
    ...
    for index, raw in enumerate(raw_options[:5], start=1):
        ...
        arrival_airport = text(
            raw.get("arrival_airport"),
            f"{destination} arrival point",
        )
        if (
            ...
            # Tokyo is valid in layover text or segments, but cannot replace the
            # requested first or last route endpoint.
            continue
        option = {
            "id": f"transport_{index}",
    ...
    return options if len(options) >= 2 else _fallback(
        origin,
        destination,
        transport_types,
    )
