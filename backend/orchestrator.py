...
SYNTHESIS_PROMPT = (
    ...
    "summary (2-3 sentence overview of the trip that leads with the destination and its "
    "activities/experiences; mention food or cuisine at most once, briefly — the trip is "
    "not a food tour even when the traveler picked a favorite cuisine), and "
    "schedule (array, one object per day, each with: day (int, 1-based), date (YYYY-MM-DD), "
    "title (short theme named after that day's activity, sight, or neighborhood — never "
    "after a meal or cuisine), and items (array of {time, type, title, detail})). "
