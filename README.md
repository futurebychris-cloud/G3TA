...
- an optional **Voice-guided trip setup** inside Dyslexia & Reading Support. It speaks the
  same trip-form questions one at a time, uses local Whisper to detect and label multilingual
  answers, advances after each transcription, repeats any required detail it cannot confirm,
...
The guided setup records a short clip with the browser MediaRecorder API and sends it to the local
Whisper service, which auto-detects common spoken languages such as English, Chinese, Spanish,
Russian, French, German, Portuguese, Arabic, Hindi, Japanese, and Korean. The destination shortcut
falls back to the browser Web Speech Recognition API. All spoken output—including Read
...
# One-time multilingual speech-to-text setup (downloads and caches Whisper).
