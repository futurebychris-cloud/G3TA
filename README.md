...
- browser-native Read Aloud controls with adjustable reading speed and optional Piper fallback;
...
- an optional **Voice-guided trip setup** inside Dyslexia & Reading Support. It speaks the
  same trip-form questions one at a time, uses browser speech recognition in the selected
  English or Chinese language with optional local Whisper fallback, advances after each
  transcription, repeats any required detail it cannot confirm,
  and starts planning after the final validated answer.
...
The guided setup uses the browser Web Speech Recognition API in the selected English or Chinese
language. On browsers without that API, it can record a short clip with MediaRecorder and send it
to the optional local Whisper service, which auto-detects common spoken languages such as English,
Chinese, Spanish, Russian, French, German, Portuguese, Arabic, Hindi, Japanese, and Korean. Spoken output—including Read Aloud and every
guided-setup question—uses the browser's built-in speech synthesis first, so it works on localhost
without Docker or a separate voice service. The local Piper service remains a compatibility
fallback for browsers without built-in synthesis. No paid speech service is used. The guided setup sends
...
### 3. Optional Piper voice fallback (Docker)
...
# Optional multilingual speech-to-text fallback (downloads and caches Whisper).
docker-compose -f docker-compose.voice.yml up -d
```

The bundled voices are English and Mandarin. The web app does not require this container when the
browser supports speech synthesis. See `piper/README.md` to mount an authorized custom Piper model
...
| POST | `/speech/synthesize` | render accessible spoken output through the optional local Piper fallback |
